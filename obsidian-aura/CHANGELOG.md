...
### StacyBot-Impuls — калибровка трейла (v6) + продажная сборка + HANDOFF

- **Продажные сборки** `pro-25`/`pro-20` (3 настройки покупателю, стратегия зашита const). Подтверждён динамический лот (риск-% → растёт с балансом); **исправлен баг округления** (`NormalizeLot` → OrderSend err 131 в live).
- **КАЛИБРОВКА ТРЕЙЛА (главное):** пользователь live-наблюдением вскрыл — оригинал переносит SL в безубыток при **+0.50** (+$10 на 0.20 лота), наш был +0.30 → выбивался рано. Построен **v6 — двухстадийный трейл** (широкий стоп → безубыток при `BeTrigger=0.50` → тесный трейл 0.30); профиль совпал с оригиналом (винрейт 97.9% ровно, ср.выигрыш 33 vs 34, PF 2.2). Поправка: разрыв — это трейл (порог безубытка), а не фильтр входа.
- **HANDOFF** (контекст заполнился): полный контекст+ТЗ+путь записан в секцию «HANDOFF / ТЗ» [[stacybot-impuls-strategy]] + journal `2026-06-15.md` + auto-memory. Следующий шаг: перенести трейл v6 в продакшн (v3+pro) + перепроверить риск-% картину.

## 2026-06-14

### StacyBot-Impuls — SL-развёртка + продажная сборка (pro) + фикс сайзинга

- **SL-развёртка** (2018→2026, риск 1%): SL — второй рычаг плеча (тесный SL → больший лот → больше доход И DD): SL10 ×29/DD51%, SL20 ×6.1/DD26%, SL25 ×3.7/DD23%. Поправка: фикс. тесный стоп НЕ взрывается (только SL=5/DD84%); взрывалась адаптивная v5. Не оптимизировать SL в максимум.
- **Продажная сборка:** 3 параметра покупателю (RiskPercent + MaxSpread + Magic), стратегия зашита const. Две сборки: `stacybot-impuls-pro-25` и `stacybot-impuls-pro-20`.
- **Сайзинг:** лот динамический (риск-% → растёт с балансом). **Исправлен баг округления** (`NormalizeLot`: float-мусор `MathFloor*step` → OrderSend err 131 в live). Краевые min/maxLot отмечены. TODO продажа: потолок RiskPercent, TF-guard, защита/лицензия .ex4, бренд. Разделы «SL-развёртка»/«Продажная сборка» в [[stacybot-impuls-strategy]].

### StacyBot-Impuls — рескью v2/v3: риск-% сайзинг возвращает выживаемость

- **v2 (регим-гейт по волатильности)** — отрицательный результат: фильтрует 1–3% входов, годы-обнуляторы не чинит. Урок: убийственные сделки нельзя предсказать по входной волатильности → ограничивать ущерб, не угадывать.
- **v3 (риск-% сайзинг 1%)** — перелом: макс. просадка <10% во ВСЕ годы (была 83–95%), плохие годы −4…−8%, хорошие сохранены. **Непрерывно 2018→2026: +266% (×3.66), сквозная DD 23.25%, PF 1.27 — переживает все режимы** (v1 обнулялся в 2019). Цель «готовность ко всему» достигнута сайзингом.
- v1-base заморожен (`stacy-backtest/variants/`). Добавлен раздел «Рескью-доработки v2/v3» в [[stacybot-impuls-strategy]], обновлены journal `2026-06-14.md`, auto-memory. Дальше: circuit-breaker/HTF на стрик 2022–24 + тюнинг риск-% + мульти-ТФ ATR + 99% тики.
- **v4 (circuit-breaker + HTF-фильтр) отвергнут** — нетто хуже v3 (фильтры вдвое срезают прибыль хороших лет, нет эджа во входе). Развёртка риск-%: 1%/+266%/DD23%, 2%/+1283%/DD45%, 3%/+4971%/DD62% (реком. 1–2%). **Продакшн = v3, `RiskPercent` — настраиваемый input** (реализация ≈ 0, параметр уже есть; пользователь валидирует аппетит сам). Все версии в `variants/`.

### StacyBot-Impuls — backtest-harness + эмпирика OOS: стратегия как есть НЕ выживает

- Построен изолированный backtest-harness (`Aura/stacy-backtest/`, portable-MT4 build 1473, headless через `terminal.exe smoke.ini /portable`; ключи `[Tester]` требуют префикс `Test`). Валидирован: окно апр–июн 2026 воспроизводит реконструкцию (1677 сделок, PF 2.58, DD 13.45%).
- **Monte-Carlo** списка сделок оригинала: наблюдаемые 7.32% просадки — везение порядка; перестановка тех же сделок → медиана DD ~20%, 99% ~37%, худшая ~59% капитала; ранний кластер 14 убытков обнуляет $10000.
- **Эмпирика OOS на реальной истории 2018→2026: счёт $10000 обнулён за ~15 мес** (net −9994, DD 99.94%, PF 0.74). Год-за-годом: **5 из 9 лет убыточны**; винрейт 90–98% во все годы, включая blow-up (2024: винрейт 94.7% при DD 94.7%) → винрейт бессмыслен. Убивают всплески волатильности кластерами, не боковик. Гейт Этапа 0 сработал — стратегия как есть непригодна; доработки (риск-% сайзинг + волатильно-новостной фильтр, затем HTF/ADX) = условие выживания.
- Добавлен раздел «Эмпирическая проверка OOS» в [[stacybot-impuls-strategy]] (таблица по годам). Обновлены frontmatter, journal `2026-06-14.md`, auto-memory.

### StacyBot-Impuls — оценка качества подхода + проект-источник переведён в АРХИВ

- Дана **оценка качества торгового подхода** (специалист по алготрейдингу) — раньше в vault была только спецификация «что это», без вердикта «насколько хорошо». По отчёту Тестера оригинала: **отрицательная асимметрия** (средний выигрыш +$34 vs убыток −$676, ≈20:1), **98% винрейт — эффект выхода** (перенос в безубыток), а не входа; **точка безубытка по винрейту ≈95.2%** → запас ~2.7 пп над разорением. Не скам (убыток ограничен стопом, нет мартингейла), но **хрупкая**: окно 2.5 мес = один режим, 90% modelling завышает винрейт, микро-прибыль уязвима к спреду/свопу. **Вердикт 3/5**; условия доверия — walk-forward 3–5 лет, Monte-Carlo, форвард-демо, риск-% сайзинг + spread/news-guard.
- Добавлен раздел «Оценка качества торгового подхода» в [[stacybot-impuls-strategy]]. Проверено, что спецификация/метод/безопасность/сравнение уже перенесены в vault + auto-memory.
- **Проект-источник `stacybot-recovery/` помечен 🗄️ АРХИВОМ** (реверс завершён, знания перенесены в Aura): создан `stacybot-recovery/ARCHIVE.md`, обновлены frontmatter + секция «Артефакты» в [[stacybot-impuls-strategy]], journal `2026-06-14.md`, auto-memory `reference_stacybot_impuls.md`, [[index]]. Артефакты остаются read-only на месте.

### StacyBot-Impuls — реверс завершён, занесён в vault как кандидат на внедрение в Aura

- Сторонний обфусцированный MT4-эксперт `stacybot-impuls.obf.ex4` (исходник утерян при атаке) реверс-извлечён до спецификации + чистого `.mq4`. Метод динамический (в обход обфускации): отчёт Стратегического тестера + трасса Frida + офлайн-подбор средней по 65000 баров. Проект — `stacybot-recovery/` (вне репо Aura).
- **Восстановлено:** трендовый trailing-scalper XAUUSD M30 — вход на открытии бара по **EMA(9) close** (buy выше / sell ниже), фикс-лот 0.3 (единственный параметр), SL 25 / TP 6, трейлинг (безубыток → шаг ~0.30), до 2 позиций. Сверка бок-о-бок: поведенчески эквивалентен (винрейт ~98%, PF ~2.4 ↔ 2.66), входит ~на 24% чаще (фильтр слабых сетапов не извлекаем из офлайн-данных).
- **Снятые риски:** кастом-индикаторов нет (рекурсивный реверс не нужен); эксперт безопасен — в сеть не ходит, персистентность не пишет (вся сеть = терминал RoboForex).
- Создано: [[stacybot-impuls-strategy]] (спецификация + метод + безопасность + сравнение с [[auragrid-impulse-strategy]] + заметки по внедрению). Обновлены [[index]], [[auragrid]] MOC. Journal `2026-06-14.md`. Auto-memory: указатель на страницу. **Следующая фаза — внедрение в Aura (новый `strategy_type`).**

---

## 2026-06-04

### perf-v2 B+C: off-loop логи + write-on-change + executor (ветка perf/v2-loop-unblock, ⏳ demo)

- Пользователь выбрал штурм-варианты **B + C** (с учётом ошибок 06-04). Реализовано на ветке `perf/v2-loop-unblock` (от `a57ac59`), 2 коммита, полный набор **1317 passed**, запушено.
- **B** (`69c7aed`): B0 — `poll_heartbeat` несёт `on_tick_p50/p99/max_ms` + `persist_writes` (**измерить живой bottleneck до правок**); B1 — `_AsyncEmitHandler` уводит запись **файла** bot.log в фоновый поток (per-line disk-flush снят с торгового loop), **stdout синхронный** (видимость крашей); B2 — write-on-change персистентность (UPSERT только при изменении).
- **C** (`59c15dc`): NO_MONEY/TRADE_DISABLED убраны из retryable (устойчивые состояния, как MARKET_CLOSED 06-03); per-tick SL-modify → one-shot. **Широкий «placements one-shot» ОТВЕРГНУТ** (он отключил бы on_retry-репрайс = риск худших заполнений). D (off-thread) gated на метрики B0.
- Без дедупа тиков и без переписывания `scan_all` (виновники регрессий 06-04). Обновлены [[auragrid-performance-strategy]] (секция «Перезаход v2» + статус реализации), journal `2026-06-04.md` (Часть 5). **Следующий шаг: demo-прогон на волатиле → метрики → решение про D.**

### largest-lot-first: обслуживание корзины по убыванию лота (SL + закрытие)

- Пользователь (наблюдение): при закрытии сетки ~7 позиций видна асинхронность (стопы/закрытие по одной). Запрос — минимум: SL начинать с самого большого лота (риск от медленности); максимум: подумать про ускорение order-execution.
- Корневая причина асинхронности — структурная: пакет `MetaTrader5` без batch/async, каждый `order_send` синхронный блокирующий round-trip, на корзине N ног = N последовательных вызовов на одном потоке.
- **Сделано (минимум):** `ProfitTrailer.apply_sl_to_all` и `Protection.close_channel` теперь обслуживают позиции по убыванию объёма (тай-брейк по тикету — новые раньше). Намеренный отход от mq5:1729 (reversed), в духе [[adr-002-trail-rework-mq5-parity-departure]]. Меняется только порядок обхода; SL/счётчики/retry идентичны. Тесты порядка добавлены (объёмы различают сортировку от reversed). Полный bot-набор **1311 passed**.
- **Максимум — НЕ реализовано осознанно:** параллельный order_send отвергнут (MetaTrader5 не потокобезопасен); реальное ускорение (выделенный single-thread executor, расцепляющий on_tick от обслуживания ордеров) отложено как ADR-gated + demo-validated — чтобы не повторить откаченные 2026-06-04 perf-регрессии.
- Создано: [[adr-005-largest-lot-first-ordering]]; обновлены [[auragrid-trading-core]], journal `2026-06-04.md` (Часть 4), [[index]]. Код в ветке `fix/shelve-perf-restore-stability`.

### demo-валидация финального MSI по живым логам: ✅ торговля корректна (стоп — ручной)

- Пользователь поставил финальную сборку (PR #37), `aura-desktop` непрерывно с 18:58:56 устройства (=10:59Z =13:59 терминала). Просьба досконально проверить логи. Фактически — demo-валидация, что требовали части 1/2. Логи `%APPDATA%\Aura\logs\` (ребренд → `Aura`); разбор delegated 2 агентам + прямые `Get-Process`/WER.
- ✅ **Защита невиновна, торговля корректна:** 0 Tauri-ERROR/Traceback/stderr, лицензия активна (expiry 19.06, 0 Unauthorized — «401/403» в grep ложные), Grid 168 монотонных trail-SL / 50 channel_reset / 0 фантомов / 0 retries_exhausted. **Bug B (часть 1) подтверждён исправленным на живом** (Impulse storm 3261→0). 17× «Invalid stops» 10016 — безвредны (позиции защищены/закрывались). 602 spread_abnormal = корректная фильтрация.
- **Остановка в 11:27Z — ручная** (стоп стратегии пользователем, подтверждено). Сначала ошибочно заподозрил «тихую смерть» (после стопа нет `python.exe`, окно вхолостую reconnect) — но это штатно после ручного стопа. **Урок: не диагностировать «креш» без позитивного доказательства; Tauri-bridge не логирует exit-code бота → стоп и креш по логам неотличимы (кандидат на мелкую доработку).** Top-level `bot.log` загрязнён pytest-артефактами (исключён из анализа).
- Обновлены: [[auragrid-incidents-log]] (2026-06-04 часть 3 — валидация PASSED + урок), journal `2026-06-04.md` (Часть 3). Анализ без правок кода. Grid + Bug B = PASSED на живом; Impulse live-валидация (Bug A/C) ещё не закрыта.

### perf-рефакторинг: регрессии торгового цикла → откат Ярусов 0–2 (PR #37)

- Пользователь (вторая жалоба того же дня): «сетка не всегда > минимума, софт сильно медленнее + лаг, закрытие по кнопке по чуть-чуть, трейлинг медленный». Диагноз: сборка содержит perf (poll_heartbeat несёт perf-поля); git blame — код сетки/трейла не менялся, менялся темп `on_tick` (дедуп тиков Ярус 1) + `scan_all` (Ярус 2b) + async-логи. Дедуп гейтит per-tick операции → регрессии. Perf делался ради исходной жалобы на лаги, но дал обратный эффект и не валидировался вживую.
- **Откат** (`18f18eb`): 5 торговых ярусов (0,1,2a,2b,2c) → checkpoint `461904b`. Ярус 4 (UI) оставлен. Защита (Фаза 2/6) + Impulse-фиксы сохранены. 6 hot-path файлов из 461904b + переналожен Aura-нейминг (`git diff 461904b` = только нейминг). bot 1309 passed. Ветка `fix/shelve-perf-restore-stability` = полная продаваемая сборка.
- Обновлены: [[auragrid-incidents-log]] (2026-06-04 часть 2 + Prevention), [[auragrid-performance-strategy]] (баннер «откачено»), journal, auto-memory. **Требует demo-валидации пересобранного MSI.**
...
- Репо `auragrid` чист и запушен (HEAD `461904b`) → правильный артефакт = именованный чекпоинт-тег, не пустой коммит.
- Baseline верифицирован зелёным во всех слоях: Python `pytest` **1306 passed** / 1 skipped / 16 xfailed / 2 xpassed / 0 failed; `tsc --noEmit` чисто; Vitest **24/24**; `cargo check` exit 0.
